...
A suspend may also be asked to tag the snapshot it takes. Tags are owned and
addressed by an Atespace, so the same tag name may exist in different
Atespaces. A tag is an immutable alias and retention pin: it holds its own copy
of the external snapshot, made at creation, so it outlives the Actor that took
it, and publishing it permits reuse from other Atespaces without changing its
...
Deleting an actor deletes the external snapshot it owns, and one an interrupted suspend left behind, before the record goes away. Snapshots it borrowed from a tag are left to that tag.
